...
//            return File(context.getExternalFilesDir("call_recording"), "recording.amr").absolutePath
        ...
        fun getSavedAudioSource(context: Context): AudioSource {
            val audioSource: AudioSource = AudioSource.valueOf(PreferenceManager.getDefaultSharedPreferences(context).getString("recordingSource", AudioSource.VOICE_RECOGNITION.name)!!)
            return audioSource
        }
    ...
    fun playRecording() {
        if (player != null)
            stopPlayRecoding()
        val recordingFilePath = getFilePath(context)
